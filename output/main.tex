...
% Tartalomjegyzék és számozási mélység: chapter + section
...
Az OnlyFix szoftvercsomag egy nyílt forráskódú, MIT licenc alatt publikált járműszerviz-menedzsment rendszer \cite{ref2}, amelynek célja a hagyományos, papíralapú szervizkezelési folyamatok kiváltása egy egységes, digitális jegykezelő (\emph{ticketing}) platformmal \cite{ref1}. A rendszer lehetővé teszi, hogy a gépjármű-tulajdonosok, szerelők és rendszerüzemeltetők hatékonyan kezeljék a járművekkel kapcsolatos hibajegyeket webes valamint natív asztali és mobil kliensről egyaránt. A szoftvercsomag két fő komponensből áll:

\begin{itemize}
\item \textbf{OnlyFix webalkalmazás} (randomUSR56/onlyfix) --- egy Laravel 12 keretrendszerre épülő monolitikus alkalmazás Vue.js 3 frontend réteggel és Inertia.js összekötő réteggel, valamint egy RESTful API réteggel külső kliensek kiszolgálására.
\item \textbf{Admin MAUI kliens alkalmazás} (randomUSR56/Admin) --- egy .NET 10 MAUI keretrendszerre épülő natív kliens alkalmazás, amely a Laravel backend RESTful API végpontjaira támaszkodva biztosít adminisztrációs hozzáférést.
\end{itemize}
...
\section{Vue.js és Inertia.js (webes kliens)}
...
\section{MAUI kliens alkalmazás}
...
Az OnlyFix RESTful API-jának dokumentálásához a Swagger (hivatalos nevén OpenAPI Specification) eszközkészletet alkalmazzuk \cite{ref21}. A Swagger egy interaktív, böngészőből elérhető felületet biztosít, amelyen keresztül az összes API végpont --- azok paraméterei, kérés- és válaszstruktúrái, valamint a szükséges autentikáció --- áttekinthető és közvetlenül kipróbálható, anélkül hogy külön HTTP-kliens eszközre (pl.\ Postman) volna szükség.

\begin{table}[ht]
	\centering
	\begin{tabular}{ll}
		\hline
		\textbf{Konfiguráció} & \textbf{Swagger elérési URL} \\ \hline
		DNS engedélyezve (ajánlott) & \url{http://onlyfix.local/swagger} \\
		DNS nélkül (localhost) & \url{http://localhost/swagger} \\ \hline
	\end{tabular}
	\caption{A Swagger felület elérési URL-jei az alkalmazás inicializálási módjától függően}
	\label{tab-swagger}
\end{table}

A Swagger felület elérési URL-je az alkalmazás inicializálásának módjától függ. \textbf{DNS-sel inicializált} környezetben (amikor a rendszer a \texttt{onlyfix.local} helyi doménnevet használja) a felület a \url{http://onlyfix.local/swagger} címen érhető el. \textbf{DNS nélkül} (azaz csak localhost-alapú konfigurációban) a felület a \url{http://localhost/swagger} címen érhető el.

\section{MySQL adatbázis}
...
Ezúton szeretnénk köszönetünket kifejezni a \textbf{Miskolci SZC Kandó Kálmán Informatikai Technikum} oktatóinak, akik áldozatos és kitartó munkájukkal hozzájárultak Vizsgaremekünk gördülékeny megvalósításához. Köszönjük a szakmai útmutatást, a fejlesztési folyamat során tanúsított türelmet és az értékes visszajelzéseket, amelyek jelentős mértékben hozzájárultak projektünk minőségéhez. Hálásak vagyunk azért az elkötelezett pedagógiai szemléletért, amellyel tanáraink végigkísérték munkánkat a tervezéstől a megvalósításig, és amelynek köszönhetően az elméleti ismeretek valódi, alkalmazható tudássá válhattak.

\vspace{1cm}

Külön köszönetet szeretnénk mondani a \textbf{Miskolc Holding Önkormányzati Vagyonkezelő Zrt.} duális képzési programjának mentorainak és vezetőinek, akik a szakmai gyakorlat teljes időszaka alatt folyamatos szakmai támogatást és inspiráló munkahelyi környezetet biztosítottak számunkra. Tapasztalataik és iránymutatásuk nélkülözhetetlen szerepet játszottak abban, hogy valódi vállalati kontextusban is mélyíthettük tudásunkat. Különösen szeretnénk megköszönni, hogy a vállalat hajlandó volt és lehetőséget talált arra, hogy a Vizsgaremek bemutatójának idejére infrastruktúrát --- hálózati erőforrásokat és munkakörnyezetet --- bocsásson rendelkezésünkre, ami nagyban hozzájárult az éles körülmények között megvalósított, igényes prezentációhoz.
...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=\textwidth]{figures/onlyfix}
	\caption{Az OnlyFix adatbázis ER diagramja}
	\label{fig:er-diagram}
\end{figure}

\begin{figure}[ht!]
	\centering
	\includegraphics[width=12cm]{figures/03-admin-dashboard}
	\caption{Admin irányítópult vázrajza}
	\label{fig:admin-dashboard}
\end{figure}

\begin{figure}[ht!]
	\centering
	\includegraphics[width=12cm]{figures/04-user-dashboard}
	\caption{Felhasználói irányítópult vázrajza}
	\label{fig:user-dashboard}
\end{figure}

\begin{figure}[ht!]
	\centering
	\includegraphics[width=12cm]{figures/05-mechanic-dashboard}
	\caption{Szerelői irányítópult vázrajza}
	\label{fig:mechanic-dashboard}
\end{figure}

\begin{figure}[ht!]
	\centering
	\includegraphics[width=\textwidth]{figures/mvp_specification}
	\caption{MVP specifikáció és brainstorming tábla}
	\label{fig:mvp-spec}
\end{figure}
...
\bibitem{ref21}
SmartBear Software, ``Swagger --- API Documentation \& Design Tools for Teams,'' [Online]. Available: \url{https://swagger.io/docs/}. [Hozzáférés dátuma: 31 03 2026].
